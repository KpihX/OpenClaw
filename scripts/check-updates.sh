...
# Fetch silencieux en arrière-plan si l'intervalle est dépassé
if [ ! -f "$CHECK_FILE" ] || [ $((NOW - $(stat -c %Y "$CHECK_FILE" 2>/dev/null || echo 0))) -gt $INTERVAL ]; then
    (cd "$REPO_DIR" && git fetch -q origin main >/dev/null 2>&1) &
    touch "$CHECK_FILE"
fi

# Vérification du nombre de commits de retard
